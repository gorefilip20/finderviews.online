...
function registerOpportunityRoutes(app) {
  ...
  app.get("/api/community-jobs", async (req, res) => { const now = Date.now(); const maxAge = 90 * 24 * 60 * 60 * 1000; const allJobs = await readJsonStore(COMMUNITY_JOBS_STORE_PATH, []); const active = allJobs.filter((job) => job.status === "open" && (!job.expiresAt || Date.parse(job.expiresAt) > now) && (now - Date.parse(job.createdAt)) < maxAge).sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt)); const role = typeof req.query.role === "string" ? req.query.role.trim().toLowerCase() : ""; const country = typeof req.query.country === "string" ? req.query.country.trim().toLowerCase() : ""; const filtered = active.filter((job) => { if (role && role !== "all hiring roles") { const searchable = (job.title + " " + job.description + " " + (job.industry || []).join(" ")).toLowerCase(); if (!searchable.includes(role)) return false; } if (country && country !== "worldwide") { const geo = (job.country + " " + job.state + " " + job.city).toLowerCase(); if (!geo.includes(country)) return false; } return true; }); res.json({ jobs: filtered.slice(0, 200), total: filtered.length, refreshedAt: new Date().toISOString() }); });
  app.post("/api/community-jobs", async (req, res) => { const user = await authenticateApiUser(req, res); if (!user) return; const body = req.body || {}; const title = typeof body.title === "string" ? body.title.trim().slice(0, 240) : ""; const company = typeof body.company === "string" ? body.company.trim().slice(0, 240) : ""; const description = typeof body.description === "string" ? body.description.trim().slice(0, 5000) : ""; const applyEmail = typeof body.applyEmail === "string" && /^\S+@\S+\.\S+$/.test(body.applyEmail) ? body.applyEmail.trim() : ""; const applyUrl = typeof body.applyUrl === "string" && /^https?:\/\//.test(body.applyUrl) ? body.applyUrl : ""; const country = typeof body.country === "string" ? body.country.trim().slice(0, 80) : ""; const state = typeof body.state === "string" ? body.state.trim().slice(0, 100) : ""; const city = typeof body.city === "string" ? body.city.trim().slice(0, 100) : ""; const jobType = typeof body.jobType === "string" ? body.jobType.trim().slice(0, 60) : "Full-time"; const level = typeof body.level === "string" ? body.level.trim().slice(0, 60) : "Not specified"; const salaryRange = typeof body.salaryRange === "string" ? body.salaryRange.trim().slice(0, 100) : ""; const industry = Array.isArray(body.industry) ? body.industry.filter((v) => typeof v === "string").map((v) => v.trim().slice(0, 120)).slice(0, 5) : []; const expiryDays = typeof body.expiryDays === "number" && body.expiryDays >= 7 && body.expiryDays <= 90 ? body.expiryDays : 30; if (!title || !company || description.length < 20) return res.status(400).json({ error: "Title, company, and a description of at least 20 characters are required." }); if (!applyEmail && !applyUrl) return res.status(400).json({ error: "Provide an application email or URL so candidates can apply." }); const allJobs = await readJsonStore(COMMUNITY_JOBS_STORE_PATH, []); const job = { id: "cjob-" + Date.now() + "-" + Math.random().toString(36).slice(2, 8), postedBy: user.openId, posterName: user.name || "", title, company, description, applyEmail, applyUrl, country, state, city, jobType, level, salaryRange, industry, status: "open", createdAt: new Date().toISOString(), expiresAt: new Date(Date.now() + expiryDays * 24 * 60 * 60 * 1000).toISOString() }; allJobs.unshift(job); await writeJsonStore(COMMUNITY_JOBS_STORE_PATH, allJobs.slice(0, 5000)); res.status(201).json({ job }); });
  app.put("/api/community-jobs/:id", async (req, res) => { const user = await authenticateApiUser(req, res); if (!user) return; const allJobs = await readJsonStore(COMMUNITY_JOBS_STORE_PATH, []); const idx = allJobs.findIndex((job) => job.id === req.params.id && job.postedBy === user.openId); if (idx === -1) return res.status(404).json({ error: "Job not found or you are not the owner." }); const body = req.body || {}; if (typeof body.status === "string" && ["open", "closed", "filled"].includes(body.status)) allJobs[idx].status = body.status; if (typeof body.title === "string") allJobs[idx].title = body.title.trim().slice(0, 240); if (typeof body.description === "string" && body.description.trim().length >= 20) allJobs[idx].description = body.description.trim().slice(0, 5000); allJobs[idx].updatedAt = new Date().toISOString(); await writeJsonStore(COMMUNITY_JOBS_STORE_PATH, allJobs); res.json({ job: allJobs[idx] }); });
  app.delete("/api/community-jobs/:id", async (req, res) => { const user = await authenticateApiUser(req, res); if (!user) return; const allJobs = await readJsonStore(COMMUNITY_JOBS_STORE_PATH, []); await writeJsonStore(COMMUNITY_JOBS_STORE_PATH, allJobs.filter((job) => !(job.id === req.params.id && job.postedBy === user.openId))); res.json({ success: true }); });
  app.post("/api/community-jobs/:id/apply", async (req, res) => { const user = await authenticateApiUser(req, res); if (!user) return; const body = req.body || {}; const resume = typeof body.resume === "string" ? body.resume.trim().slice(0, 15000) : ""; const coverNote = typeof body.coverNote === "string" ? body.coverNote.trim().slice(0, 2000) : ""; const applicantEmail = typeof body.applicantEmail === "string" && /^\S+@\S+\.\S+$/.test(body.applicantEmail) ? body.applicantEmail.trim() : ""; const applicantName = typeof body.applicantName === "string" ? body.applicantName.trim().slice(0, 120) : user.name || ""; if (!resume || resume.length < 20) return res.status(400).json({ error: "Include your resume (at least 20 characters)." }); if (!applicantEmail) return res.status(400).json({ error: "Provide your email so the employer can reach you." }); const allJobs = await readJsonStore(COMMUNITY_JOBS_STORE_PATH, []); const job = allJobs.find((j) => j.id === req.params.id); if (!job) return res.status(404).json({ error: "Job not found." }); if (job.status !== "open") return res.status(400).json({ error: "This position is no longer accepting applications." }); if (!job.applyEmail) return res.status(400).json({ error: "This job has no application email configured." }); if (!process.env.RESEND_API_KEY || !process.env.OUTREACH_FROM_EMAIL) return res.status(503).json({ error: "Email sending is not configured. Set RESEND_API_KEY and OUTREACH_FROM_EMAIL on Hostinger." }); const emailBody = "New application for: " + job.title + " at " + job.company + "\n\nApplicant: " + applicantName + "\nEmail: " + applicantEmail + (coverNote ? "\n\nCover note:\n" + coverNote : "") + "\n\n--- RESUME ---\n" + resume + "\n\n---\nSent via Finderviews Community Job Board"; const emailRes = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: "Bearer " + process.env.RESEND_API_KEY, "Content-Type": "application/json" }, body: JSON.stringify({ from: process.env.OUTREACH_FROM_EMAIL, to: [job.applyEmail], reply_to: applicantEmail, subject: "Application: " + job.title + " - " + applicantName, text: emailBody }) }); if (!emailRes.ok) return res.status(502).json({ error: "Could not deliver your application. Try the apply URL instead." }); res.json({ success: true, message: "Application sent to " + job.company + "." }); });
